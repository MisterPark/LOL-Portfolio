...
HierarchyLoader::HierarchyLoader(LPDIRECT3DDEVICE9 pGraphicDev, const WCHAR* pPath)
...
HierarchyLoader::~HierarchyLoader(void)
{

}

STDMETHODIMP HierarchyLoader::CreateFrame(THIS_ LPCSTR Name, LPD3DXFRAME* ppNewFrame)
{
...
STDMETHODIMP HierarchyLoader::CreateMeshContainer(THIS_ LPCSTR Name,
	...
	LPD3DXMESHCONTAINER* ppNewMeshContainer)
{
	...
	ULONG	dwNumFaces = pMesh->GetNumFaces();	// 메쉬가 지닌 폴리곤 개수를 반환

	pDerivedMeshContainer->pAdjacency = new ULONG[dwNumFaces * 3];
	memcpy(pDerivedMeshContainer->pAdjacency, pAdjacency, sizeof(ULONG) * dwNumFaces * 3);

	ULONG	dwFVF = pMesh->GetFVF();
	...
	if (0 != NumMaterials)
	{
		...
		for (ULONG i = 0; i < pDerivedMeshContainer->NumMaterials; ++i)
		{
	...
	pDerivedMeshContainer->pFrameOffsetMatrix = new Matrix[pDerivedMeshContainer->dwNumBones];
	ZeroMemory(pDerivedMeshContainer->pFrameOffsetMatrix, sizeof(Matrix) * pDerivedMeshContainer->dwNumBones);

	pDerivedMeshContainer->ppFrameCombinedMatrix = new Matrix * [pDerivedMeshContainer->dwNumBones];
	ZeroMemory(pDerivedMeshContainer->ppFrameCombinedMatrix, sizeof(Matrix*) * pDerivedMeshContainer->dwNumBones);

	pDerivedMeshContainer->pRenderingMatrix = new Matrix[pDerivedMeshContainer->dwNumBones];
	ZeroMemory(pDerivedMeshContainer->pRenderingMatrix, sizeof(Matrix) * pDerivedMeshContainer->dwNumBones);
...
STDMETHODIMP HierarchyLoader::DestroyMeshContainer(THIS_ LPD3DXMESHCONTAINER pMeshContainerToFree)
{
...
void HierarchyLoader::Allocate_Name(char** ppName, const char* pFrameName)
{
	if (nullptr == pFrameName)
		return;

	UINT		iLength = strlen(pFrameName);
...
HierarchyLoader* HierarchyLoader::Create(LPDIRECT3DDEVICE9 pGraphicDev, const WCHAR* pPath)
{
	return new HierarchyLoader(pGraphicDev, pPath);
}
